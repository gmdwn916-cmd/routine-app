...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[구석의 "음력" 토글로 양력/음력 전환]/근무·요일)은
    ...
    - **중요(★) 토글·원본 날짜/요일/근무 정보 줄은 없앰(2026-07-16, 사용자
      요청)**: 예전엔 입력칸 옆에 중요 표시 별(promote-important-btn) 버튼이
      있었고, "반복으로 전환" 모드(한 번짜리 할 일을 반복으로 바꿀 때)일
      땐 제목 옆에 원래 날짜·요일·근무를 한 줄로 보여줬는데(예: "7월 16일
      (수) 주간") 둘 다 화면을 복잡하게 만든다는 이유로 지움. **중요 표시가
      필요하면 만든 뒤 "반복 할일" 탭에서 그 줄을 눌러 나오는 메뉴(반복/
      중요/수정)의 "중요"로 하면 됨** — 이 화면 자체에 다시 별표 버튼을
      만들지 말 것. 내부적으로 promoteImportant 변수는 그대로 남겨뒀는데,
      이미 중요 표시가 있던 반복 할일을 이 화면에서 열어 다른 값(텍스트·
      규칙 등)을 고쳐도 그 표시가 조용히 사라지지 않게 하려는 용도일 뿐
      (이 화면에서 값을 바꿀 방법은 없음, 읽기만 함) — 다시 이 값을 UI로
      노출하려 하지 말 것.
