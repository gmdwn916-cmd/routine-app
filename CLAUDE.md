...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[구석의 "음력" 토글로 양력/음력 전환]/근무·요일)은
    ...
    - **"반복으로 전환"해도 원래 한 번짜리 기록은 안 지움(2026-07-16
      ...
      씀). **부작용(의도된 트레이드오프, 버그 아님)**: 새 반복 규칙이 우연히
      원래 그 날짜에도 맞아떨어지면, 그 날은 원래 한 번짜리 기록과 반복
      항목이 같이 보여서 같은 할 일이 중복으로 보일 수 있음 — 사용자가
      이 가능성을 알고 이 방식을 선택함, "중복 안 되게 자동으로 걸러줘" 같은
      요청이 다시 들어오기 전까지는 손대지 말 것.
