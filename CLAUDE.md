...
- weatherRegion?: string (2026-07-19 추가) — 오늘 탭 날씨가 어느 지역 기준인지
  (WEATHER_REGIONS의 id, 예: 'seoul'). 없으면 'seoul' 기본값. 위치 권한 없이
  사용자가 오늘 탭의 날씨 표시를 눌러 고른 지역만 저장(전국 17개 시·도
  단위) — 자세한 내용은 "화면 구성" 섹션의 "오늘 탭 날씨 표시" 참고.
...
- **오늘 탭 날씨 표시 추가(2026-07-19)**: 사용자 요청 — 이 앱은 "교대근무
  ...
  답을 듣고 그 수준으로만 구현함(오늘 탭 맨 위 카드에 아이콘+지역+온도
  한 줄만 추가, 그 이상의 상세 예보·시간대별 예보 등은 안 만듦). **기기
  위치 권한은 안 씀** — 안드로이드 위치 권한 추가·웹뷰 권한 프롬프트 처리
  등 이 작은 기능치고 배보다 배꼽이 커져서, 대신 사용자가 한 번 골라두는
  지역(`state.weatherRegion`, 기본값 서울, 전국 17개 시·도 단위)만 씀 —
  `#td-weather` 버튼(오늘 탭 상단 카드 안)을 누르면 지역 선택 팝업
  (`#weather-region-overlay`, 다른 바텀시트 오버레이와 같은 공용 패턴 +
  `.shift-picker-item` 재사용)이 뜨고, 고르면 `state.weatherRegion`에 저장.
  **날씨 API는 API 키가 필요 없는 Open-Meteo를 씀** — 이 앱은 서버 없는
  정적 파일(`index.html` 하나, GitHub Pages)이라 어떤 키를 코드에 넣어도
  그대로 공개 노출되므로, 키 자체가 필요 없는 서비스가 이 프로젝트 구조에
  맞음(한국천문연구원 음력 데이터를 쓰는 것과 같은 이유로 이미 이 앱은
  "키 없는 공개 API/데이터"만 쓰는 관례가 있었음). 현재 기온 + WMO 날씨
  코드를 짧은 한글 설명(맑음/흐림/비/눈 등)+이모지로 변환해서 보여줌.
  **캐시**: 메모리 변수(`weatherCache`, 새로고침되면 초기화)에 지역별로
  20분 caching — `renderToday()`가 호출될 때마다(할 일 체크 등으로도 자주
  불림) 매번 새로 fetch하지 않게 함. **실패해도 조용히 무시** —
  `updateTodayWeatherUI()`를 `renderToday()`에서 `try/catch`로 감싸고, fetch
  실패(오프라인 등) 시에도 `.catch`로 삼켜서 이 부가 기능 하나 때문에 오늘
  탭 전체 렌더링이 깨지면 안 된다는 원칙을 지킴 — 데이터가 없으면
  `.td-weather`는 그냥 빈 채로(`:empty`시 `display:none`) 안 보임. **위젯에는
  아직 안 넣음**(웹/오늘 탭에만 적용, 사용자가 요청하면 그때 따로 검토).
