...
- **크기 4x1~4x5 리사이즈(2026-07-21 재설계)** — 처음엔 항상 정확히 2주
  ...
  계기)에서 현재 위젯 세로 크기(dp)로 몇 줄이 들어갈지 어림잡아
  (`estimateVisibleWeeks()`, 다른 위젯들의 `estimateVisibleRows()`와 같은
  성격의 어림 계산 — 예전 2주 고정일 때의 `minHeight=110dp`를 역산해 헤더+
  패딩 32dp, 한 줄당 39dp로 추정) 그만큼만 `VISIBLE`, 나머지는 `GONE`으로
  둠. **GONE인 줄은 부모 LinearLayout의 weight 배분에서 자동으로 빠지므로,
  보이는 줄들이 남는 세로 공간을 알아서 나눠 채움** — 별도 weight 재계산
  코드가 필요 없었음. `widget_schedule_info.xml`도 `minHeight`를 110dp→70dp로
  낮추고(1줄도 허용) `maxResizeHeight="240dp"`를 추가(API 31+에서만 적용,
  그 미만 버전은 이 속성을 무시하지만 렌더링 로직 자체가 최대 5줄로
  clamp돼 있어서 문제없음).
