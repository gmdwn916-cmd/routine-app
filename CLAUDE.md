...
- 오늘 날짜·요일·근무를 위젯 2·3처럼 크게 보여주고, 그 아래 오늘의 반복
  ...
  쓴 위젯.
- **첫 줄(날짜)에 기온·날씨 이모지도 이어붙임(2026-07-19 추가)**: 오늘 탭의
  날씨 표시(위 "오늘 탭 날씨 표시" 참고)가 정한 `weatherCache`를 공휴일
  이름과 완전히 같은 방식으로 `buildTodayWidgetPayload()`의 `dateLabel`
  문자열 끝에 그대로 이어붙임("7월 19일 일요일 24° ☁️") — 네이티브
  레이아웃에 새 칸을 안 만들고, 이 위젯의 "근무 계산은 항상 JS가 끝내서
  결과만 넘긴다" 원칙 그대로 문자열 하나로 처리. 기온+이모지만 붙이고
  "해/흐림/비/눈" 글자는 안 붙임(오늘 탭 날씨 표시와 달리 여기는 사용자가
  "기온과 날씨 이모지"라고 정확히 지정). `weatherCache`가 아직 없으면
  (위치 권한 거부·오프라인 등) 그냥 안 붙음 — 이 위젯도 다른 곳과 같은
  "날씨는 부가 정보, 없어도 나머지는 정상 표시" 원칙을 따름. **날씨가 새로
  갱신되는 시점(`updateTodayWeatherUI()`가 fetch 성공했을 때)에도
  `pushTodayWidgetToWidget()`을 부르도록 새 push 시점을 추가함** — 다른
  위젯 push는 전부 "저장 시점"에 걸려 있는데, 날씨는 시간이 지나면 저절로
  바뀌는 값이라 저장 이벤트가 없어도 위젯에 반영되게 하려면 이 시점이
  따로 필요했음.
