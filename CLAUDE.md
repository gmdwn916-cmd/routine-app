...
- 홈 화면 "빠른 할일 추가" 위젯 (네이티브 전용, 2026-07-13): 위젯을 탭하면 작은
  ...
  - 위젯 크기는 처음엔 1x1(가장 작음)에 아이콘(+ 모양, ic_widget_add.xml)만 있고
    ...
    조금 내려달라"는 요청 반영)이었다가, "여전히 이상하다"는 후속 피드백으로
    다시 gravity="center"(그 절반의 정중앙)로 바꿔서 지금은 "위쪽 절반 정중앙
    글씨 + 아래쪽 절반 정중앙 아이콘"으로 최종 정리됨 — 아이콘은 원래부터
    scaleType="centerInside"라 이미 그 절반의 정중앙이었어서 안 건드림.
    레이아웃도 아이콘 하나만 있던
