...
- **설정 탭 메뉴 묶음(2026-07-18 추가)**: 항목 7개가 구분 없이 한 줄로
  ...
  재검토.
- **"교대 주기" → "내 근무"로 개명 + "내 팀 이름" 관리를 여기로 이사
  (2026-07-18)**: 설정 메뉴 키(`cycle`)와 화면 id(`#sd-cycle`)는 코드상
  그대로 두고(다른 이름 개명들과 같은 관례 — id/키는 안 바꾸고 라벨만
  바꿈) 사용자에게 보이는 이름만 "내 근무"로 바꿈. 원래 "팀 근무" 화면
  안에 있던 "내 팀 이름 만들기/변경" UI(입력칸+버튼)를 이 화면(#sd-cycle)
  맨 위 새 카드(`#my-team-name-card`, renderMyTeamNameCard())로 옮김 —
  "내 팀 이름도 내 근무를 이루는 정보"라는 사용자 판단. "팀 근무" 화면은
  이제 `state.myTeamName`이 비어 있으면 입력칸 없이 "먼저 '내 근무'에서
  입력해 주세요" 안내만 보여줌(그 화면에서 직접 입력 못 함).
  **teamNameEditing**(모듈 전역 변수)이 "변경" 버튼을 눌러 편집 중인지
  추적 — openSettingsDetail('cycle')이 이 화면을 열 때마다 항상 false로
  리셋 후 renderMyTeamNameCard()를 다시 부름(날짜 상세·미배치 등 다른
  임시 UI 상태를 화면 열 때마다 리셋하는 것과 같은 원칙). **이름을 확정할
  때만 state.myTeamName을 바꾸도록 고침** — 예전엔 "변경" 버튼을 누르는
  순간 이름을 바로 빈 값으로 지우고 저장까지 해버려서, 입력칸에 기존
  이름이 안 보이고 취소할 방법도 없었음(사용자가 신고).
