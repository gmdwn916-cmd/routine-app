...
- **페이지 넘기기**: 위젯 크기로 1주/2주를 정하던 예전 방식(리사이즈 기반)은
  ...
  넘김. SCHEDULE_PAGE_COUNT(=2, 즉 4주치)만 미리 계산해 넘겨받음 — 그 범위
  밖은 앱을 열어야 갱신됨(달력 위젯과 같은 한계). 참고로 보여준 다른 앱은
  스와이프로 넘기는 방식이었지만, RemoteViews에서 스와이프 페이지는
  훨씬 복잡해서(AdapterViewFlipper+RemoteViewsService 필요) 버튼 탭 방식으로
  단순화함 — 나중에 스와이프로 바꿔달라는 요청이 없으면 이대로 유지.
  앱이 새 자료를 보낼 때마다 페이지 위치는 0(이번 주부터)으로 리셋됨.
