...
- 카테고리는 설정 탭에서 사용자가 직접 추가·수정(이름/아이콘/색)·삭제 가능
  (state.categories — CATEGORIES_DEFAULT는 최초 1회 씨앗 데이터일 뿐, 이후엔
  안 씀). getCategories()/getCat(id)로 조회 — getCat은 없는 id면 배열 순서와
  무관하게 고정된 회색 "기타" 모양으로 폴백(카테고리를 삭제해도 그 카테고리를
  쓰던 할 일이 깨지지 않게). 최소 1개는 남아야 해서 마지막 하나는 삭제 버튼이
  숨겨짐. 근무 색·디자인 토큰은 여전히 코드 상단 상수로 관리(사용자 수정 불가).
