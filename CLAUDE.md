...
- **오늘 탭 날씨 표시 추가(2026-07-19)**: 사용자 요청 — 이 앱은 "교대근무
  ...
  - **지역 결정 우선순위**: `state.weatherRegion`(사용자가 수동으로 고른
    지역, 있으면 최우선) → 세션 중 캐시된 GPS 좌표(20분 이내) → 그 세션에서
    위치 요청이 이미 거부됐으면(`gpsDeniedThisSession`) 재시도 없이 바로
    대체 지역(서울) → 아직 시도 안 했으면 위치 권한 요청 후 좌표로 조회.
    **표시되는 "지역" 이름은 실제 GPS 좌표를 기존 17개 시·도 좌표 목록과
    비교해 가장 가까운 곳의 이름으로 근사**(`nearestWeatherRegionName` —
    역지오코딩 API 없이, 이미 있던 지역 좌표 테이블을 그대로 재사용한
    단순 최근접 매칭). 날씨 자체는 근사한 지역 좌표가 아니라 **실제 GPS
    좌표로 조회**(더 정확함) — 이름 표시만 근사, 데이터는 정확.
