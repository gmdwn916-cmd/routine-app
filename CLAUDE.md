...
3) 배치: 항목당 남은 질문은 하나만(넣기의 OX가 첫 갈림길이므로).
   ...
   - 반복 O → "어떤 날마다?" : 버튼 2개만 제공("기간" / "근무·요일") — 탭이
     ...
     b. 기간으로 고르기: 들어가면 안에 "기간"/"음력" 하위 버튼 2개가 또 있음.
        - 기간(기본): "3개월마다" 등 년/개월/일 간격 (근무 주기와 무관, 실제
          달력 날짜로 계산. 전환 시작일 = 그 할 일의 원래 날짜, 새로 만들
          때는 오늘)
        - 음력: "매년 음력 3월 15일마다" (부모님 생신 등 극소수 용도).
          "음력" 버튼을 누르면 원래 날짜의 음력이 자동으로 숫자 칸(월/일)에
          채워져 있어서, 마음에 안 들면 숫자만 밀어서 고치고 그대로면 바로
          확인만 눌러도 추가됨.
...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[하위: 기간/음력]/근무·요일)은 openRepeatEditor() 하나로 공용
