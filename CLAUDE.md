...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[구석의 "음력" 토글로 양력/음력 전환]/근무·요일)은
    ...
    - **"기간"/"근무·요일" 탭 다시 누르면 선택 취소(2026-07-16 추가)**: 이미
      켜져 있는 모드 버튼을 한 번 더 누르면 setPromoteMode(null)로 선택이
      풀림(양쪽 버튼 다 꺼지고 두 섹션 다 숨겨짐) — **단, 새로 만드는 중일
      때만**(!repeatEditorTargetEvent) 이렇게 동작하고, 이미 저장된 반복
      할일을 편집하는 중이면 두 번 눌러도 아무 일도 안 일어남(사용자가 직접
      확인한 동작 — 반복 규칙이 없는 반복 할일은 데이터 구조상 존재할 수
      없어서, 편집 중에는 취소해도 "반복 안 함"으로 만들 방법이 없고 그냥
      무시함). 새로 만들다가 취소하면 syncPromoteEditorToState()가 규칙 없음
      상태로 조용히 저장을 보류하는 기존 동작을 그대로 씀(위 저장 규칙과
      동일한 경로) — 별도의 "취소" 로직을 새로 안 만듦.
...
- 카테고리는 설정 탭에서 사용자가 직접 추가·수정(이름/아이콘)·삭제 가능
  ...
  - **할 일 줄·반복 할일 카드의 카테고리 아이콘 칸(.item-icon/.routine-icon)은
    "기타"일 때 아예 안 그림**(2026-07-16 재조정 — 처음엔 글자만 빈 채로
    두었는데 그래도 둥근 배지 자리가 빈 채로 남는 게 어색하다는 요청으로,
    buildTodoRow/buildRoutineCard 둘 다 cat.id==='etc'면 iconEl 자체를
    만들지도 붙이지도 않도록 바꿈 — flex라 자리 자체가 완전히 사라짐).
    사용자가 만든 다른 카테고리가 이모지 없이 저장된 경우엔 여전히 점(●)
    표시 — "기타"만의 예외임, 다른 아이콘 없는 카테고리까지 숨기지 말 것.
