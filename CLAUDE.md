...
- **체크 반응 영역은 체크칸(item_check)만** — 텍스트(item_text)와 나머지
  ...
  **버그(수정 완료, 2026-07-18)**: 체크칸을 뺀 나머지(글자·빈 줄 포함)를
  ...
  겹치게 함.
  **버그(수정 완료, 2026-07-18, 같은 날 재조사)**: 위 수정 이후에도 "목록 바로
  밑 빈 줄"은 눌리는데 "그보다 더 아래쪽 빈 공간"은 여전히 무반응이라는
  재신고 — 원인이 달랐음. today_list(ListView)는 `layout_weight=1`로 위젯의
  남은 공간을 전부 차지하는데, 실제 항목이 적을 때 그 박스 안에는 항목이
  다 채우지 못한 빈 여백이 남음 — 근데 그 여백도 여전히 ListView 자신의
  영역(박스 안)이라, 탭하면 ListView가 통째로 삼켜버려서(스크롤 제스처
  처리를 위해 항상 그럼) 부모의 PendingIntent(위젯 머리글에 걸린 것)로
  안 넘어감 — "빈 줄 하나(FILLER_COUNT)"까지만 눌리고 그 밑은 안드로이드
  구조상 원천적으로 반응이 안 되는 것이었음(이전엔 이 한계를 "의도된
  트레이드오프"로 문서에만 남기고 넘어갔었는데, 이번 최종 점검에서 사용자가
  실제로 다시 겪어서 제대로 고침). 고침: 안드로이드 12(API 31) 이상 기기
  전용으로 `widget_today_v31.xml`을 새로 만듦(같은 뷰 id 재사용, 그
  미만 기기는 예전 `widget_today.xml` 그대로 — TodayWidgetProvider.
  updateOne()이 `Build.VERSION.SDK_INT`로 분기) — 이 레이아웃은 today_list에
  weight를 아예 안 주고, `RemoteViews.setViewLayoutHeight()`(API 31+ 전용
  메서드)로 "실제 항목 수 + 빈 줄 1개"만큼만 정확한 높이(TODAY_ROW_HEIGHT_DP
  = 30dp 어림값 × 줄 수)를 매번 다시 계산해서 지정함. 그 아래 진짜로 남는
  여백은 새로 추가한 `today_list_filler`(목록에 안 속하는 평범한 TextView,
  weight=1이라 LinearLayout이 남는 공간을 전부 자동으로 이 뷰에 몰아줌)가
  차지하고, 여기엔 위젯 머리글과 똑같은 PendingIntent를 직접 걺 — 목록
  소속이 아닌 일반 뷰라 항상 확실히 반응함. **왜 이 방식인지**: 위젯의
  실제 화면 크기(dp)를 추정해서 계산하는 방식은 이미 예전에 시도했다가
  실제 기기에서 크게 틀려서 실패한 적이 있음(FILLER_COUNT 항목의
  2026-07-16 기록 참고, 다시 시도하지 말라고 적어둔 방식) — 이번 방식은
  위젯의 "전체 크기"를 전혀 몰라도 되고, 오직 "항목이 몇 개인지"(우리가
  이미 정확히 아는 값)만으로 목록 높이를 정하고 나머지는 LinearLayout의
  weight 계산에 그대로 맡기는 것이라 그 실패를 반복하지 않음. **API 31
  미만 기기는 이 개선이 적용되지 않고 예전 한계가 남아있음**(setViewLayoutHeight
  자체가 그 밑 버전에 없음) — 필요해지면 그때 다른 방식을 추가로 검토.
...
- **버그(수정 완료, 2026-07-18)**: 오늘 위젯과 똑같은 원인으로, 항목이 적을
  때 목록(inbox_list) 안쪽 빈 여백을 눌러도 반응이 없던 문제 — 오늘 위젯의
  해법(위 위젯 4 항목의 2026-07-18 기록 참고)을 그대로 이식함:
  `widget_inbox_v31.xml`을 새로 만들어 API 31(안드로이드 12) 이상 기기에서만
  씀(그 미만은 예전 `widget_inbox.xml` 그대로, InboxWidgetProvider.
  updateOne()이 `Build.VERSION.SDK_INT`로 분기) — inbox_list에서 weight를
  빼고 `RemoteViews.setViewLayoutHeight()`로 실제 항목 수(`count`) ×
  `INBOX_ROW_HEIGHT_DP`(30dp 어림값)만큼만 높이를 지정, 남는 여백은 새로
  추가한 `inbox_list_filler`(목록 소속이 아닌 평범한 뷰, weight=1)가 차지해서
  거기에 앱 열기 PendingIntent를 직접 걺. **오늘 위젯과 다른 점**: 오늘
  위젯(TodayRemoteViewsFactory)은 목록 자체에 빈 줄 하나(FILLER_COUNT)를 더
  채워 넣는 로직이 있어서 높이 계산에 `+1`을 더했는데, 이 위젯
  (InboxRemoteViewsFactory)은 그런 빈 줄 로직이 아예 없어서(getCount()가
  항상 실제 항목 수 그대로) `+1` 없이 count를 그대로 씀 — 이 차이를 헷갈려서
  다시 +1을 넣지 말 것.
