...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[구석의 "음력" 토글로 양력/음력 전환]/근무·요일)은
    ...
    - **"근무·요일" 반복 중복 감지 + 합치기(2026-07-17 추가,
      syncPromoteEditorToState 안 dupShiftTarget)**: "반복으로 전환" 중일 때만
      (repeatEditorSourceEvent 있음, "새로 만들기"는 대상 아님 — 사용자가
      명시적으로 확정) 적용. 지금 만들려는 규칙이 "근무·요일"(type:'shift')
      방식이고, **같은 텍스트 + 같은 type:'shift'인 반복 할일이 이미 있으면**
      `confirm('이미 반복 할일에 "..."이(가) 있어요. 하나로 합칠까요?')`로
      확인함. **"기간"(interval/lunar) 방식은 이 확인 대상이 아님** —
      기간-근무든 기간-기간이든 항상 별개 할일로 관리하기로 확정(다시 이
      확인을 기간 쪽에 넣지 말 것). 확인창 결과:
      - **"예"(합침)**: 기존 반복 할일의 근무×요일 표 선택(expandGroupsToGridSelection
        으로 복원) + 지금 화면에서 고른 칸을 합집합으로 합쳐서 기존 항목의
        `.repeat`을 다시 계산(collapseGridSelectionToGroups)해서 덮어씀 —
        **기존 항목의 텍스트·카테고리·중요 표시는 그대로 유지**(사용자가
        직접 확인 — 텍스트는 이미 같아서 상관없고, 카테고리·중요는 새로
        전환하던 쪽 값으로 안 덮음). 전환 중이던 원본(한 번짜리) 기록은 여느
        때처럼 보존하고, 그 날짜만 기존 반복 할일의 excludedDates에 추가(대상만
        새로 만든 항목이 아니라 기존 항목이라는 점만 다름, 나머지는 "반복
        전환 시 원본 기록 보존" 규칙과 동일). 이 화면 자체는 그 순간부터
        그 기존 반복 할일을 편집하는 상태로 바뀜(repeatEditorTargetEvent =
        기존 항목, repeatEditorSourceEvent = null, 제목 "반복 할 일 편집"으로
        바뀜, renderShiftMatrix()로 합쳐진 칸 상태를 바로 보여줌) — 새 항목은
        따로 안 만들어짐.
      - **"아니요"(별개로 관리)**: 지금 고른 칸은 그대로 둔 채 원래 하던 대로
        새 반복 할일을 별개로 만듦(평소 생성 경로 그대로 진행) — 합치기를
        거부한 것뿐이지 선택을 취소하는 게 아님.
      - 이 확인은 "근무·요일" 규칙이 처음 만들어지는 그 순간(!repeatEditorTargetEvent
        일 때) 딱 한 번만 뜸 — 그 뒤로는(합쳤든 새로 만들었든) repeatEditorTargetEvent
        가 채워져서 이후 칸 조작은 그냥 그 항목을 갱신하는 일반 경로로 감,
        같은 세션 안에서 반복해서 뜨지 않음.
