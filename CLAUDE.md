...
3) 배치: 항목당 남은 질문은 하나만(넣기의 OX가 첫 갈림길이므로).
   ...
   - 반복 O → "어떤 날마다?" : 아래 3가지 방식 제공(버튼 3개 — 탭이 지저분해지지
     않도록 음력은 별도 버튼이 아니라 "기간" 안의 하위 선택으로 넣어둠).
     ...
     c. 기간으로 고르기: 들어가면 안에 "기간"/"음력" 하위 버튼 2개가 또 있음.
...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[하위: 기간/음력]/근무로/요일로)은 openRepeatEditor() 하나로 공용
