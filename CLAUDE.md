...
- **오늘 탭 날씨 표시 추가(2026-07-19)**: 사용자 요청 — 이 앱은 "교대근무
  ...
  - **짧게 탭 = 기기 기본 날씨 앱 열기, 꾹 누르기 = 지역 수동 선택
    (2026-07-19 추가)**: 처음엔 탭 한 번으로 지역 선택 팝업이 열렸는데,
    사용자가 "예보를 눌렀을 때 기본 날씨 어플을 켜줄 수 있나?"라고 요청 —
    미배치 목록의 "짧은 탭/꾹 누르기" 상태 기계(`setupInboxItemDrag`)와 같은
    원리로 재구성함: `touchstart`에서 0.5초 타이머를 걸고, 그 안에 손을 떼면
    (`longPressed` 그대로 false) 뒤이어 브라우저가 만드는 진짜 `click`
    이벤트가 그대로 발생해 `openDefaultWeatherApp()`을 부름 — 타이머가 다 돌
    때까지 누르고 있으면(`longPressed=true`) `openWeatherRegionPicker()`가
    열리고, 그 뒤 `touchend`에서 `preventDefault()`로 뒤따라올 가짜 click을
    막아서 날씨 앱까지 같이 안 열리게 함. 데스크톱 마우스 클릭은 롱프레스
    구분이 없어 항상 날씨 앱 열기로만 감(터치 전용 기기 대상 기능이라
    문제없음, 다른 롱프레스 기능들도 마찬가지로 터치 전용).
    **"기본 날씨 앱"을 여는 표준 안드로이드 API는 없음** — 대신 런처가
    "날씨 앱"을 찾을 때 쓰는 표준 카테고리 `Intent.CATEGORY_APP_WEATHER`로
    등록된 앱을 찾아서 그대로 여는 방식(`WidgetBridgePlugin.openWeatherApp()`,
    이 프로젝트가 직접 짠 네이티브 플러그인에 새 메서드로 추가 — npm 패키지
    아님, 이미 있는 `WidgetBridge` 플러그인 재사용) — 삼성/스톡 안드로이드
    등 대부분의 기본 날씨 앱이 이 카테고리로 등록돼 있음. 등록된 앱이
    없으면(드묾) 조용히 아무 일도 안 함(`opened:false`만 돌려주고 JS도
    별다른 처리 없이 넘어감 — 이 부가 기능의 "실패해도 조용히" 원칙과 동일).
    웹(GitHub Pages)에서는 `getWidgetBridge()`가 애초에 null이라
    `openDefaultWeatherApp()`도 그냥 아무 것도 안 함("기본 날씨 앱" 개념
    자체가 웹엔 없음).
