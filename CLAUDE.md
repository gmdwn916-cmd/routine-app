...
- **크기 4x1~4x5 리사이즈(2026-07-21 재설계)** — 처음엔 항상 정확히 2주
  ...
  늘려서 다른 위젯들과 같은 "최대치 선언 + GONE" 방식), `onAppWidgetOptionsChanged`
  (이 위젯엔 이번에 처음 추가— 오늘/미배치 위젯의 리사이즈 대응과 같은
  계기)에서 현재 위젯 세로 크기(dp)로 몇 줄이 들어갈지 계산해(`estimateVisibleWeeks()`)
  그만큼만 `VISIBLE`, 나머지는 `GONE`으로 둠. **GONE인 줄은 부모 LinearLayout의
  weight 배분에서 자동으로 빠지므로, 보이는 줄들이 남는 세로 공간을 알아서
  나눠 채움** — 별도 weight 재계산 코드가 필요 없었음.
  **`estimateVisibleWeeks()` 계산식 재조정(같은 날) — "4x1이면 정확히 1주,
  4x2면 정확히 2주"(원래 보여주던 그대로) 요청**: 처음엔 "헤더+패딩
  32dp, 한 줄당 39dp"라는 어림 상수로 추정했는데, 정확히 딱 맞아떨어지길
  원해서 안드로이드 위젯 크기 공식을 그대로 역산하는 방식으로 바꿈 — 이
  프로젝트의 다른 위젯 minWidth/minHeight 값들이 전부 공식 문서의 "셀
  크기 = 70×n-30"(dp) 규칙을 따르고 있음을 확인함(2x2 위젯들의 110dp =
  70×2-30, 1x1 위젯의 40dp = 70×1-30, 이 위젯 자체의 minWidth 250dp =
  70×4-30) — 그러니 반대로 `주 수 = round((minHeightDp+30)/70)`으로 계산하면
  1행=40dp→1주, 2행=110dp→2주, 3행=180dp→3주, 4행=250dp→4주, 5행=320dp→5주로
  항상 정확히 맞아떨어짐(어림값이 아니라 확정값). `widget_schedule_info.xml`도
  이 공식에 맞춰 `minHeight`를 40dp(1주 허용)로, `maxResizeHeight`를
  320dp(5주)로 맞춤(API 31+에서만 적용되는 속성, 그 미만 버전은 무시하지만
  렌더링 로직 자체가 최대 5줄로 clamp돼 있어서 문제없음). **앞으로 위젯
  크기로 줄/칸 수를 계산하는 비슷한 기능을 만들 때, "정확히 맞아떨어져야
  하는" 요구사항이면 이 공식(`70×n-30`)을 어림값 대신 쓸 것** — 이
  프로젝트의 모든 위젯 크기 선언이 이미 이 공식 하나로 통일돼 있어서
  역산이 항상 가능함.
